...
// 📐 constants
import { TABLE_COLUMNS, HEADER_BUTTONS, TABLE_ACTIONS } from './country.constants';
@Component({
...
export class CountryComponent {
  private readonly rawClients = signal<ICountry[]>([
    { id: 1, nameInArabic: 'العربية', nameInEnglish: 'Arabic', Code: 'ar', phoneCode: '964' },
    { id: 2, nameInArabic: 'الإنجليزية', nameInEnglish: 'English', Code: 'en', phoneCode: '44' },
    { id: 3, nameInArabic: 'الفرنسية', nameInEnglish: 'French', Code: 'fr', phoneCode: '33' },
  ]);
  ...
  protected readonly filteredClients = computed(() => {
    ...
    return (
      this.rawClients().filter((client) => client.nameInArabic.toLowerCase().includes(query)) ||
      this.rawClients().filter((client) => client.nameInEnglish.toLowerCase().includes(query))
    );
  });
  ...
  protected onTableActionTrigger(event: TableActionEvent<ICountry>): void {
  ...
  protected onRowsSelected(selectedClients: ICountry[]): void {
  ...
  private openEditClientModal(client: ICountry): void {
