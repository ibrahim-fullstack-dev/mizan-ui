...
import { IPurchaseInvoices } from './purchase-invoices.types';
...
} from './purchase-invoices.constant';
@Component({
...
export class PurchaseInvoicesComponent {
  private readonly rawBillsInvoices = signal<IPurchaseInvoices[]>([
    {
      id: 1,
  ...
  protected readonly filteredPurchaseInvoices = computed(() => {
  ...
  protected readonly totalBillsInvoicesCount = computed(
    () => this.filteredPurchaseInvoices().length,
  );
  ...
  protected onTableActionTrigger(event: TableActionEvent<IPurchaseInvoices>): void {
  ...
  protected onRowsSelected(selectedClients: IPurchaseInvoices[]): void {
  ...
  private openEditClientModal(client: IPurchaseInvoices): void {
