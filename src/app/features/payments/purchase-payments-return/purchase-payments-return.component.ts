...
// 📐 constants
import { TABLE_COLUMNS, HEADER_BUTTONS, TABLE_ACTIONS } from './purchase-payments-return.constant';
@Component({
...
export class PurchasePaymentsReturnComponent {
  private readonly rawExpenses = signal<IPurchasePaymentReturn[]>([
    {
      id: 1,
  ...
  protected readonly filteredsales = computed(() => {
    ...
    return this.rawExpenses().filter((client) => client.supplierName.toLowerCase().includes(query));
  });
  ...
  protected onTableActionTrigger(event: TableActionEvent<IPurchasePaymentReturn>): void {
  ...
  protected onRowsSelected(selectedClients: IPurchasePaymentReturn[]): void {
  ...
  private openEditClientModal(client: IPurchasePaymentReturn): void {
