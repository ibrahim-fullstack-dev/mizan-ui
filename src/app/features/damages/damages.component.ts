...
// 📐 constants
import { TABLE_COLUMNS, HEADER_BUTTONS, TABLE_ACTIONS } from './damages.constant';
@Component({
  selector: 'app-client',
  standalone: true,
...
export class DamagesComponent {
  ...
  protected readonly filteredDamages = computed(() => {
    ...
    return this.rawDamages().filter((client) => client.name.toLowerCase().includes(query));
  });

  protected readonly totalClientsCount = computed(() => this.filteredDamages().length);
  ...
  protected onTableActionTrigger(event: TableActionEvent<IDamage>): void {
  ...
  protected onRowsSelected(selectedClients: IDamage[]): void {
  ...
  private deleteClient(id: number): void {
    this.rawDamages.update((current) => current.filter((client) => client.id !== id));
  }
  ...
  private openEditClientModal(client: IDamage): void {
