...
export class PurchaseOrdersComponent {
  private readonly rawOffers = signal<IPurchaseOrder[]>([
    {
      id: 1,
  ...
  protected readonly filteredPurchaseOrders = computed(() => {
    ...
    return this.rawOffers().filter((orders) => orders.supplierName.toLowerCase().includes(query));
  });

  protected readonly totalPurchaseOrders = computed(() => this.filteredPurchaseOrders().length);
  ...
  protected onTableActionTrigger(event: TableActionEvent<IPurchaseOrder>): void {
  ...
  protected onRowsSelected(selectedClients: IPurchaseOrder[]): void {
  ...
  private openEditClientModal(client: IPurchaseOrder): void {
