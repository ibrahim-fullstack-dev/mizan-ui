...
// 📐 constants
import { TABLE_COLUMNS, HEADER_BUTTONS, TABLE_ACTIONS } from './transfer-orders.constant';
@Component({
  selector: 'app-client',
  standalone: true,
...
export class TransferOrdersComponent {
  private readonly rawBranches = signal<ITransferOrder[]>([
    {
      id: 1,
  ...
  protected readonly filteredBranches = computed(() => {
    ...
    return this.rawBranches().filter((branch) => branch.sourceBranch.toLowerCase().includes(query));
  });
  ...
  protected onTableActionTrigger(event: TableActionEvent<ITransferOrder>): void {
  ...
  protected onRowsSelected(selectedClients: ITransferOrder[]): void {
  ...
  private openEditBranchModal(client: ITransferOrder): void {
